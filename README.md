...
The trajectory map needs a Mapbox **public** token (`pk.*`) in `.env` (gitignored). `npm run deploy` builds with that local env, then pushes only `build/` to `gh-pages` — the `.env` file itself is never committed. The `pk.*` string is still inlined into the JS bundle (normal for client-side Mapbox); restrict the token by URL in the Mapbox dashboard. Do not use a secret token (`sk.*`) in the frontend. Without a token, the rest of the site still runs; the map shows a setup hint instead.
